...
class HISSDLearnerContinueTrain:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, task: str, use_external_skill=False, return_losses=False):
        """增强train函数支持返回损失值"""
        ...
        if self.adaptation:
            # 替换三个独立的训练函数为一个整合的函数
            v_loss, reward_loss, rec_loss, total_loss, sample_losses = self.train_adaptation(batch, t_env, episode_num, task)
    ...
    def train_reward(self, batch: EpisodeBatch, t_env: int, episode_num: int, task: str):
        ...
        for i in range(self.c):
            ...
            reward_out_h = self.mac.forward_planner_feedforward(
                out_h, additional_input=agent_inputs, forward_type="reward", task=task,adaptation=self.adaptation
            )
            mac_reward.append(reward_out_h)
    ...
    def train_adaptation(self, batch: EpisodeBatch, t_env: int, episode_num: int, task: str):
        ...
        # guidance_loss = th.tensor(0.0, device=self.device)
        ...
        # ====== 阶段1：首先计算所有时间步的normal_value ======
        normal_values = []
        self.mac.init_hidden(batch.batch_size, task)
        
        for t in range(seq_length):
            # 计算普通值函数（不使用skill）
            normal_value = self.mac.forward_value(batch, t=t, task=task)
            normal_values.append(normal_value)
        
        normal_values = th.stack(normal_values, dim=1)  # [bs, seq_length, n_agents, 1]
        
        # ====== 阶段2：计算skill-based值函数和奖励预测 ======
        # 重新初始化hidden state，避免与normal_value计算共用hidden state
        mac_reward = []
        mac_value = []
        target_mac_value = []
        ...
        for t in range(seq_length):
            ...
            # 计算dec_loss，参考train_vae中的实现
            if t < seq_length - self.c and t%self.c == 0:
                # 计算当前时间步的动作预测
                    agent_inputs = self.mac._build_inputs(batch, t=t, task=task)
                    act_agent_outs = self.mac.forward_planner_feedforward(
                        out_h, additional_input=agent_inputs, forward_type="action", task=task, adaptation=self.adaptation
                    )
                    ...
                    if mask_sum > 0:
                        with th.no_grad():  # 不需要计算梯度
                            ...
                            dec_loss += curr_dec_loss
            
            # 1. 使用skill预测奖励
            reward_out_h = self.mac.forward_planner_feedforward(
                out_h, additional_input=states, forward_type="reward", task=task,adaptation=self.adaptation
            )
            mac_reward.append(reward_out_h)
            
            # 2. 使用skill预测值函数
            value_out_h = self.mac.forward_planner_feedforward(
                out_h, additional_input=states, forward_type="value", task=task,adaptation=self.adaptation
            )
            mac_value.append(value_out_h)
            
            # 3. 计算目标网络的基于skill的值函数
            with th.no_grad():
                target_value_out_h = self.target_mac.forward_planner_feedforward(
                    out_h, additional_input=states, forward_type="value", task=task
                )
                target_mac_value.append(target_value_out_h)
            
            # 4. 世界模型重建损失计算（每self.c步计算一次）
            if t % self.c == 0 and t + self.c < seq_length:
        ...
        for t in range(seq_length):
            ...
            with th.no_grad():
                target_value = self.target_mac.forward_value_skill(batch.batch_size, target_mac_value[t], task=task)
                target_skill_values.append(target_value)
        
        skill_values = th.stack(skill_values, dim=1)  # [bs, seq_length, n_agents, 1]
        target_skill_values = th.stack(target_skill_values, dim=1)  # [bs, seq_length, n_agents, 1]
        ...
        reward_preds = th.stack(reward_preds, dim=1)  # [bs, seq_length, 1]
        ...
        # 使用mixer聚合多智能体值函数 - 在整个序列上计算
        if self.mixer is not None:
            mixed_skill_values = self.mixer(
                skill_values, batch["state"], self.task2decomposer[task]
            )
            mixed_normal_values = self.mixer(
                normal_values, batch["state"], self.task2decomposer[task]
            )
            with th.no_grad():
                target_mixed_skill_values = self.target_mixer(
                    target_skill_values, batch["state"], self.task2decomposer[task]
                ).detach()
        else:
            mixed_skill_values = skill_values.sum(dim=2)
            mixed_normal_values = normal_values.sum(dim=2)
            with th.no_grad():
                target_mixed_skill_values = target_skill_values.sum(dim=2).detach()
        
        # 准备计算TD损失的掩码 - 只在有效范围内计算损失
        valid_mask = mask[:, :-self.c]  # 排除最后self.c步
        
        # 为每个时间步创建termination mask
        term_masks = []
        for t in range(seq_length - self.c):
            # 检查从t到t+self.c-1之间的所有时间步是否有终止
            term_mask = th.ones_like(terminated[:, 0])
            for j in range(self.c):
                if t + j < seq_length:
                    term_mask = term_mask * (1 - terminated[:, t+j])
            term_masks.append(term_mask)
        
        term_masks = th.stack(term_masks, dim=1)  # [bs, seq_length-self.c]
        
        # 计算TD目标 - 使用self.c为偏移
        bootstrap = self.main_args.gamma**self.c * term_masks * target_mixed_skill_values[:, self.c:]
        td_targets = cs_rewards[:, :-self.c] + bootstrap
        
        # 计算TD误差 - 只在有效范围内计算
        td_error = mixed_skill_values[:, :-self.c] - td_targets
        masked_td_error = td_error * valid_mask
        
        # 计算值函数损失
        value_loss = th.sum(masked_td_error**2) / (valid_mask.sum() + 1e-9)
        
        # 计算普通值函数的TD损失 - 也只在有效范围内计算
        normal_td_error = mixed_normal_values[:, :-self.c] - td_targets
        masked_normal_td_error = normal_td_error * valid_mask
        normal_value_loss = th.sum(masked_normal_td_error**2) / (valid_mask.sum() + 1e-9)
        
        # 计算奖励预测损失 - 只在有效范围内计算
        reward_mask = mask[:, :-self.c]
        reward_pred_loss = F.mse_loss(
            reward_preds[:, :-self.c].reshape(-1, 1),
            cs_rewards[:, :-self.c].reshape(-1, 1),
            reduction="none"
        ) * reward_mask.reshape(-1, 1)
        
        # 保存每个样本的损失值，用于计算优先级
        sample_value_losses = (masked_td_error**2).reshape(batch.batch_size, -1).sum(axis = -1) / valid_mask.reshape(batch.batch_size, -1).sum(axis = -1)
        sample_reward_losses = reward_pred_loss.reshape(batch.batch_size, -1).sum(axis=-1)/(reward_mask.reshape(batch.batch_size, -1).sum(axis=-1) + 1e-8)
        
        # 计算每个样本的总损失（用于优先级）
        sample_losses = sample_value_losses + sample_reward_losses
        
        # 计算平均损失
        reward_pred_loss = reward_pred_loss.sum() / (reward_mask.sum() + 1e-8)
        
        # 平均重建损失（世界模型）- 已在循环中累积，按循环次数平均
        rec_steps = max(1, (seq_length-self.c) // self.c)
        rec_loss = rec_loss / rec_steps
        ...
        if (
            ...
            # self.logger.log_stat(f"{task}/guidance_loss", guidance_loss.item(), t_env)
            self.logger.log_stat(f"{task}/reward_pred_loss", reward_pred_loss.item(), t_env)
            self.logger.log_stat(f"{task}/rec_loss", rec_loss.item(), t_env)
        ...
        if self.use_wandb:
            wandb.log({
                ...
                f"{task}/normal_value_loss": normal_value_loss.item(),
                # f"{task}/guidance_loss": guidance_loss.item(),
                f"{task}/reward_pred_loss": reward_pred_loss.item(),
                f"{task}/rec_loss": rec_loss.item(),
        ...
        # 返回损失值字典，包括样本级别的损失 - 新增代码
        return value_loss, reward_pred_loss, rec_loss, total_loss, {
            'value_loss': sample_value_losses.detach().cpu().numpy(),
            'reward_loss': sample_reward_losses.detach().cpu().numpy(),
        }
