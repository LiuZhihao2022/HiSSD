...
class MAGumbelLearner:
    ...
    def train_vae(
        ...
        task: str,
        ssl_loss=None,
        use_external_skill=False, # 新增参数，允许外部指定skill_index
    ):
        ...
        while t < batch.max_seq_length - self.c:
            ...
            # 得到当前batch的skill embedding和skill index，支持external_skill_index
            agent_outs, _, skill_index, _ = self.mac.forward_planner(
                batch, t=t, task=task, actions=actions[:, t], hrl=True, skill_index_out=True,
                external_skill_index=actions[:,t] if use_external_skill else None # 新增：允许外部指定skill_index
            )
    ...
    def train_value(self, batch: EpisodeBatch, t_env: int, episode_num: int, task: str):
        ...
        #         / mask[:, : -self.c].sum()
        #     )
        # ---------------------------------专门针对ma gumbel muzero的更改--------------------
        # 因为现在是一个纯online的环境，所以不要使用implicit Q-learning的权重了
        value_loss = th.mean((masked_td_error**2).sum()) / mask[:, : -self.c].sum()
        loss = value_loss
    ...
    def train_planner(
        ...
        for t in range(batch.max_seq_length - self.c):
            ...
            training=True,
            loss_out=True,
            external_skill_index= actions[:, t] if use_external_skill else None,  # 使用外部技能索引
            )
        ...
        for i in range(self.c):
            out_h, _, _, _ = self.mac.forward_planner(
                batch, t=t + i, task=task, actions=actions[:, t + i],external_skill_index= actions[:, t + i] if use_external_skill else None
            )
        ...
        for i in range(1, self.c):
            cs_rewards[:, : -self.c] += discount * rewards[:, i : -(self.c - i)]
            discount *= self.main_args.gamma
        # adaption模式不改变planner了，只改变value。得到的skill以及action decoder都是固定的
        if self.adaptation:
            # 它的意思是，不要让value的预测太保守了，像在offline里一样。现在已经是online adaption了，大胆点
            mask_sum = mask.sum()
            loss = -mixed_values.sum() / mask_sum if mask_sum > 0 else th.tensor(0.0, device=self.device)
        else:
            td_error = (
            ...
            # Calculate average for each loss in accumulated_loss_dict
            for key in accumulated_loss_dict:
                accumulated_loss_dict[key] /= (batch.max_seq_length - self.c)
                
            mask = mask.expand_as(mixed_values)
            # TODO: 不会是这些空的位置，计算的loss也参与了梯度，所以预测值被强行绑定在了0附近？
            # First apply mask to filter out invalid elements
            masked_rewards_pre = reward_pre * mask
            masked_cs_rewards = cs_rewards * mask

            # Then compute loss only on valid elements in range [:-self.c]
            valid_rewards_pre = masked_rewards_pre[:, :-self.c]
            valid_cs_rewards = masked_cs_rewards[:, :-self.c]

            # Count non-zero elements in the mask for this range
            mask_sum = mask[:, :-self.c].sum()

            if mask_sum > 0:  # Ensure division by zero is avoided
                # Sum the squared differences and divide by the number of valid elements
                reward_pred_loss += ((valid_rewards_pre - valid_cs_rewards)**2).sum() / mask_sum
            else:
                reward_pred_loss += th.tensor(0.0, device=self.device)
            
            mask_sum_td = mask[:, : -self.c].sum()
            if mask_sum_td > 0:
                td_error = (td_error * mask[:, : -self.c]).sum() / mask_sum_td
                ...
                loss = planner_loss + self.main_args.reward_pred_weight * reward_pred_loss
        ...
        if self.use_wandb:
            ...
            wandb.log(accumulated_loss_dict, step=t_env)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, task: str, use_external_skill=False):
        ...
        if self.adaptation:
            ...
            # v_loss = 0
            # TODO: 这里记得修改回来
            dec_loss, ssl_loss, infonce_loss = self.train_vae(batch, t_env, episode_num, task, use_external_skill=use_external_skill)
            ...
            self.train_planner(
                ...
                use_external_skill=use_external_skill
            )
