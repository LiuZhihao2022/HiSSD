...
import functools
import torch as th
from mctx._src.recurrent_fn import make_multiagent_recurrent_fn_gym
...
class MultiTaskHierMCTSEpisodeRunner:
    ...
    def run(self, test_mode=False, nolog=False, pretrain=False):
        ...
        while not terminated:
            if self.t % self.c_step == 0:
                # TODO: select a skill based on the current state and mcts
            pre_transition_data = {
            ...
            # TODO: action should be revised to be selected with current skill
