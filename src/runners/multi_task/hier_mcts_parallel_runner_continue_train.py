...
class HierMCTSParallelRunnerContinueTrain:
    ...
    def _batch_select_skill_with_mcts(self, env_indices, record_distribution=False):
        ...
        # 获取参考skill：使用与parallel_runner类似的方式获取skill
        # 只对需要选择新skill的环境调用select_actions
        # 这里要注意：我们不想改变mac中的hidden state，因此这里使用test_mode=True
        # 同时我们只是要获取skill_index，所以我们不使用这个返回的动作
        full_reference_skills = None
        if getattr(self.args, "use_ref_skills", False):
            _, reference_skill_indices = self.mac.select_actions(
                self.batch, 
                t_ep=self.t, 
                t_env=self.t_env, 
                task=self.task, 
            ...
            # Reshape to (batch_size, n_agents, 1)
            reference_skill_indices = reference_skill_indices.view(len(env_indices), self.n_agents, 1)
            
            # For the full batch size, creating a zero tensor
            full_reference_skills = th.zeros((self.batch_size, self.n_agents,1), dtype=th.long).to(self.args.device)
            
            # Fill in the values for the environments that need new skills
            for i, idx in enumerate(env_indices):
                full_reference_skills[idx] = reference_skill_indices[i]

        
        # 如果需要debug，可以打印参考skill
        # print(f"参考skill indices: {reference_skill_indices}")
        ...
        value_pre = self.mac.forward_value(
            ...
            t=self.t,
            task=self.task,)
        batch_state = self.batch["state"][:, self.t]
        value_pre.unsqueeze_(1)
        outer_value = self.mac.mixer(
                # value_pre, state_input, self.task_decomposer
                value_pre, batch_state.reshape(self.batch_size , 1 ,batch_state.shape[-1]), self.task_decomposer
            )
        # ------------------------------------------------------------------------
        ...
        # 批量初始化root，添加reference_skill_indices参数
        roots, experienced_thresholds, root_policy_hidden_states, root_critic_hidden_states = initialize_root(
            ...
            reference_skill_indices=full_reference_skills  # 新增：传递参考skill
        )
    ...
    def plot_distribution(self, save_dir="./results/mcts_distributions"):
        ...
        # Add entropy and distribution statistics
        ax.text(0.02, 0.95, 
                f"Prior Entropy: {prior_entropy:.4f}\n"
                f"Max Logit: {np.max(prior_logits):.4f}\n"
                f"Min Logit: {np.min(prior_logits):.4f}\n"
                f"Logit Range: {np.max(prior_logits) - np.min(prior_logits):.4f}", 
                transform=ax.transAxes, fontsize=10, verticalalignment='top',
                bbox=dict(boxstyle='round', facecolor='white', alpha=0.5))
        
        prior_logits_path = os.path.join(save_dir, f"prior_logits_ep{episode}_env{env_idx}_t{t}.png")
        plt.savefig(prior_logits_path, dpi=120, bbox_inches='tight')
        plt.close(fig)
        print(f"Prior probability and logits chart saved to {prior_logits_path}")
        
        # Save distribution data to CSV for later analysis
        try:
            import pandas as pd
            # Prepare data
            data = {
            ...
            print("Cannot import pandas, skipping CSV data saving")
