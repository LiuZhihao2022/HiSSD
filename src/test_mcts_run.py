...
# 添加MultiTaskReplayBuffer类
class MultiTaskReplayBuffer:
    """为每个任务维护单独的ReplayBuffer"""
    
    def __init__(self, task_list, task2scheme, task2groups, task2preprocess, 
                 buffer_size, batch_size, device="cpu", buffer_cpu_only=True):
        self.task_buffers = {}
        self.task_buffer_sizes = {}
        self.buffer_size = buffer_size  # 每个任务的buffer大小
        ...
        for task in task_list:
            self.task_buffers[task] = ReplayBuffer(
                ...
                max_seq_length=task2scheme[task]["state"]["vshape"][0] + 1 if isinstance(task2scheme[task]["state"]["vshape"], tuple) else 
                            task2scheme[task]["episode_limit"] + 1 if "episode_limit" in task2scheme[task] else 
                            1000,  # 默认值，如果无法确定
                preprocess=task2preprocess[task],
                device="cpu" if buffer_cpu_only else device
            )
            self.task_buffer_sizes[task] = 0
    
    def insert_episode_batch(self, task, episode_batch):
        """将episode_batch插入到对应任务的buffer中"""
        if task not in self.task_buffers:
            raise KeyError(f"Task {task} not found in MultiTaskReplayBuffer")
        
        self.task_buffers[task].insert_episode_batch(episode_batch)
        self.task_buffer_sizes[task] = self.task_buffers[task].episodes_in_buffer
...
def init_tasks(task_list, main_args, logger):
    ...
    for task in task_list:
        ...
        if getattr(task_args, "basic_action_as_skill", False):
            task_args.skill_dim = task_args.n_actions
            task2runner[task].args.skill_dim = task_args.n_actions
        # scheme适配SMACv2
        scheme = {
...
def train_online_mcts(
    ...
    for task in test_tasks:
        ...
        if mcts_network is None:
            ...
            mcts_network = PolicyRNN(
                ...
                # 对于gumbel来说，output shape和n_action是一样的
    ...
    while current_t_env < main_args.online_steps:
        # 收集在线数据
        for task in test_tasks:
            mcts_task2runner[task].save_initial_state()
            # 使用MCTS收集经验
            while True:
                # episode_data = mcts_task2runner[task].run(test_mode=False)
                episode_data = mcts_task2runner[task].test_mcts_bandit(num_trials=1)
                episode_batch, mcts_buffer, stats_info = episode_data
                
                # 将收集到的episode_batch存入learner_buffer
            ...
            # TODO: just for test
            # sample_batch_size = main_args.batch_size
