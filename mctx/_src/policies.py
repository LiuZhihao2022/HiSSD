...
from mctx._src.network import ReplayBuffer
from mctx._src.utils import compute_advantage
from mctx._src.tree import infer_batch_size
def muzero_policy(
    params: base.Params,
    ...
    *,
    ...
    temperature: float = 1.0) -> base.PolicyOutput[None]:
  """Runs MuZero search and returns the `PolicyOutput`."""
  rng_key, dirichlet_rng_key, search_rng_key = np.random.RandomState(), np.random.RandomState(), np.random.RandomState()

  # Adding Dirichlet noise.
  noisy_logits = _get_logits_from_probs(
      _add_dirichlet_noise(
          dirichlet_rng_key,
          torch.softmax(torch.tensor(root.prior_logits), dim=-1).numpy(),
  ...
  # 这个visit_probs是muzero中根据访问次数得到的一个改善策略
...
def gumbel_muzero_policy(
  ...
  Args:
    ...
    value_network_params: parameters of the value network.
  ...
  # 这里会放入k+1个动作，第k+1是为了做参考的，会一直mask掉
  root = root.replace(
  ...
  # TODO: 目前已经完成root action的选取。接下来需要完成内部action的选取
  search_tree, timing_stats = search.search(
      params=params,
      rng_key=rng_key,
      root=root,
      recurrent_fn=recurrent_fn,
...
def stochastic_muzero_policy(
  ...
  batch_size = root.embedding[0].shape[0]
  dummy_action = np.zeros([batch_size], dtype=np.int32)
  ...
  root = root.replace(
      prior_logits=np.concatenate([
          root.prior_logits,
          np.full([batch_size, num_chance_outcomes], fill_value=-np.inf)
      ], axis=-1),
      embedding=base.StochasticRecurrentState(
          state_embedding=root.embedding,
          afterstate_embedding=dummy_afterstate_embedding,
          is_decision_node=np.ones([batch_size], dtype=bool)))
...
def _add_dirichlet_noise(rng_key, probs, *, dirichlet_alpha, dirichlet_fraction):
  ...
  noise = np.random.dirichlet(
      alpha=np.full([num_actions], fill_value=dirichlet_alpha),
      size=(batch_size,))
...
def _apply_temperature(logits, temperature):
  ...
  return logits / np.maximum(tiny, temperature)
...
def _make_stochastic_recurrent_fn(
  ...
  def stochastic_recurrent_fn(
    ...
    batch_size = state.state_embedding[0].shape[0]
    ...
    output_if_decision_node = base.RecurrentFnOutput(
        prior_logits=np.concatenate([
            np.full([batch_size, num_actions], fill_value=-np.inf),
            decision_output.chance_logits], axis=-1),
        value=decision_output.afterstate_value,
        reward=np.zeros_like(decision_output.afterstate_value),
        discount=np.ones_like(decision_output.afterstate_value))
    ...
    new_state = base.StochasticRecurrentState(
        ...
        is_decision_node=np.logical_not(state.is_decision_node))

    def _broadcast_where(decision_leaf, chance_leaf):
      extra_dims = [1] * (len(decision_leaf.shape) - 1)
      expanded_is_decision = np.reshape(state.is_decision_node,
                                         [-1] + extra_dims)
      return np.where(
          expanded_is_decision,
          decision_leaf, chance_leaf)

    output = np.vectorize(_broadcast_where, excluded=[0, 1])(output_if_decision_node, output_if_chance_node)
...
def _make_stochastic_action_selection_fn(
  ...
  def _chance_node_selection_fn(
    ...
    prob_chance = torch.softmax(torch.tensor(chance_logits), dim=-1).numpy()
    argmax_chance = np.argmax(prob_chance / (num_chance + 1), axis=-1).astype(np.int32)
    return argmax_chance

  def _action_selection_fn(key: np.random.RandomState, tree: search.Tree,
                           node_index: np.ndarray,
    ...
    return np.where(is_decision, decision_selection, chance_selection)
