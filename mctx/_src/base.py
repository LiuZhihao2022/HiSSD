...
class RecurrentFnOutput:
  ...
  Attributes:
    reward (chex.Array): `[B]` an approximate reward from the state-action transition.
    discount (chex.Array): `[B]` the discount between the `reward` and the `value`.
    prior_logits (chex.Array): `[B, num_actions]` the logits produced by a policy network.
    value (chex.Array): `[B]` an approximate value of the state after the state-action transition.
    policy_hidden_states (chex.Array): Hidden states of the policy network.
    critic_hidden_states (chex.Array): Hidden states of the critic network.
    sampled_actions (chex.Array): `[B, k]` the sampled actions.
  """
  reward: chex.Array
  discount: chex.Array
...
class RootFnOutput:
  ...
  observation: `[B, num_agents, ...]` the observations of the current state.
  new_policy_hidden_states: `[B, ...]` the new policy hidden states.
  new_critic_hidden_states: `[B, ...]` the new critic hidden states.
  sampled_actions: `[B, k]` the sampled actions.
  """
  prior_logits: chex.Array
  value: chex.Array
