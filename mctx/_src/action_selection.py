...
def muzero_action_selection(
  ...
  prior_probs = torch.softmax(torch.tensor(prior_logits), dim=-1).numpy()
  policy_score = np.sqrt(node_visit) * pb_c * prior_probs / (visit_counts + 1)
  ...
  node_noise_score = 1e-7 * np.random.uniform(size=tree.num_actions)
...
GumbelMuZeroExtraDataType = TypeVar("GumbelMuZeroExtraDataType", bound=GumbelMuZeroExtraData)
...
def gumbel_muzero_interior_action_selection(
  ...
  to_argmax = _prepare_argmax_input(
      # 这里在ma-gumbel-muero中应该用advantage，但是adv=q-v, 经过softmax后值与原来相同，所以就直接用Q了
